...
### 5. Build and deploy the docker image from lesson-4/django to ECR

1. Retrieve an authentication token and authenticate your Docker client to your registry. Use the AWS CLI:

```bash
aws ecr get-login-password --region eu-central-1 | docker login --username AWS --password-stdin 683210040028.dkr.ecr.eu-central-1.amazonaws.com
```
...
```bash
docker tag lesson-5-ecr:latest 683210040028.dkr.ecr.eu-central-1.amazonaws.com/lesson-5-ecr:latest
```

4. Run the following command to push this image to your newly created AWS repository:

```bash
docker push 683210040028.dkr.ecr.eu-central-1.amazonaws.com/lesson-5-ecr:latest
```

---

### 6. Deploy the Django Application with Helm
